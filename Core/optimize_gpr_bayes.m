function [smpl,lnP,lnL,accept] = optimize_gpr_bayes( X, Y, theta0, nsamples, Method, indices )
...
lnpdf = @(theta)gpr_posterior(X,Y,theta,Method,indices);                    % Posterior distribution
...
function [lnP,lnL] = gpr_posterior(X,Y,theta,Method,indices)
...
lnL = gpr_likelihood( X, Y, theta, Method, indices );
